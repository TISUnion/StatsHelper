...
def refresh_uuid_list(server: ServerInterface):
    ...
    if os.path.isfile(constants.UUIDFilePrev):
        ...
        server.logger.info('Migrated {} uuid mapping from the previous {}'.format(
            len(uuid_file), os.path.basename(constants.UUIDFilePrev)))
    # compatibility ends
...
def show_stat(source: CommandSource, name: str, cls: str, target: str, is_uuid: bool, is_tell: bool):
    ...
    if uuid is None:
        print_message(source, tr('player_uuid_not_found', name),
                      is_tell=is_tell)
    if config.save_world_on_query:
        trigger_save_all(source.get_server())
    msg = tr('player_stat_display', name, get_display_text(
        cls, target), utils.get_stat_data(uuid, cls, target))
...
def show_rank(source: CommandSource, cls: str, target: str, list_bot: bool, is_tell: bool, is_all: bool, is_called: bool = False):
    ...
    if not is_called:
        print_message(source, tr('show_rank.title', get_display_text(
            cls, target), sum, show_range), is_tell=is_tell)
    ...
    for i in range(show_range):
        ...
        ret.append(text)
        if not is_called:
            print_message(source, utils.get_rank_color(
                i) + text, is_tell=is_tell)
...
def build_scoreboard(source: CommandSource, cls: str, target: str, title: Optional[str] = None, list_bot: bool = False):
    ...
    server.execute('scoreboard objectives remove {}'.format(
        constants.ScoreboardName))
    if title is None:
    ...
    server.execute('scoreboard objectives add {} minecraft.{}:minecraft.{} {}'.format(
        constants.ScoreboardName, cls, target, title.to_json_str()))
    for name, uuid in player_list:
        value = utils.get_stat_data(uuid, cls, target)
        if value is not None:
            server.execute('scoreboard players set {} {} {}'.format(
                name, constants.ScoreboardName, value))
...
def save_scoreboard(source: CommandSource, alias: str, cls: str, target: str, title: Optional[str] = None):
    ...
    if is_succeeded:
        ...
        source.reply(RText(tr('save_scoreboard.duplicated', alias)).c(
            RAction.run_command, f'{constants.Prefix} list').h(tr('list_scoreboard.promote')))
...
def rm_scoreboard(source: CommandSource, alias: str):
    ...
    if is_succeeded:
        ...
        source.reply(RText(tr('rm_scoreboard.not_found', alias)).c(
            RAction.run_command, f'{constants.Prefix} list').h(tr('list_scoreboard.promote')))
...
def list_quick_scoreboard(source: CommandSource, is_tell: bool):
    ...
    print_text.append(tr('list_scoreboard.summary') + RText('[+]', color=RColor.green).c(
        RAction.suggest_command, f'{constants.Prefix} save ').h('list_scoreboard.add') + '\n')
    num = 0
    if len(saved_list) == 0:
        ...
        for s in saved_list:
            num += 1
            display = tr('list_scoreboard.cls_target') + ': ' + \
                get_display_text(s.cls, s.target)
            if s.title is not None:
                display += ' §2{}§r: {}'.format(
                    tr('list_scoreboard.title'), s.title)
            print_text.append(RTextList(
                RText('[x] ', RColor.dark_red).c(RAction.suggest_command, f'{constants.Prefix} del {s.alias}').h(
                    tr('list_scoreboard.delete', s.alias)),
                RText(f'§d{s.alias}§r {display}').c(RAction.run_command, f'{constants.Prefix} scoreboard {s.alias}').h(
                    tr('list_scoreboard.show', s.alias))
            ))
...
def register_command(server: PluginServerInterface):
    def exe(node: AbstractNode, callback: Callable[[CommandContext, Arguments], Any]) -> AbstractNode:
        return node.runs(lambda src, ctx: callback(ctx, Arguments.empty())).\
            then(ArgumentEnding('args').runs(
                lambda src, ctx: callback(ctx, ctx['args'])))
    ...
    def _show_stat(ctx: CommandContext, args: Arguments):
        ...
        if isinstance(ref, Scoreboard):
            show_stat(ctx.source, ctx['player'], ref.cls, ref.target,
                      is_uuid=args.is_uuid, is_tell=args.is_tell)
        else:
            show_stat(ctx.source, ctx['player'], ref[0], ref[1],
                      is_uuid=args.is_uuid, is_tell=args.is_tell)
    ...
    def _show_rank(ctx: CommandContext, args: Arguments):
        ref = ctx['cls/alias']  # type: Union[Scoreboard, Tuple[str, str]]
        if isinstance(ref, Scoreboard):
            show_rank(ctx.source, ref.cls, ref.target, list_bot=args.is_bot,
                      is_tell=args.is_tell, is_all=args.is_all)
        else:
            show_rank(ctx.source, ref[0], ref[1], list_bot=args.is_bot,
                      is_tell=args.is_tell, is_all=args.is_all)
    ...
    def _build_scoreboard(ctx: CommandContext, title: Optional[str], args: Arguments):
        ref = ctx['cls/alias']  # type: Union[Scoreboard, Tuple[str, str]]
        if isinstance(ref, Scoreboard):
            build_scoreboard(ctx.source, ref.cls, ref.target,
                             ref.title, list_bot=args.is_bot)
        else:
            build_scoreboard(
                ctx.source, ref[0], ref[1], title, list_bot=args.is_bot)
    ...
    server.register_command(
        ...
        on_error(UnknownArgument, lambda src: src.reply(
            RText(tr('command.unknown_argument')).c(
                RAction.run_command, constants.Prefix)
        ), handled=True).
        on_child_error(UnknownQuickScoreboard, lambda src: src.reply(
            RText(tr('command.unknown_scoreboard')).c(
                RAction.run_command, f'{constants.Prefix} list')
        )).
        ...
        then(Literal('save').then(
            Text('alias').then(
                Text('cls').then(
                    ...
                    then(
                        QuotableText('title').
                        runs(lambda src, ctx: save_scoreboard(
                            src, ctx['alias'], ctx['cls'], ctx['target'], ctx['title']))
                    )
                )
            )
        )).
...
def on_load(server: PluginServerInterface, old_module):
    ...
    config = server.load_config_simple(
        constants.ConfigFile, in_data_folder=False, target_class=Config)
    Config.set_instance(config)
    HelpMessage = tr('help_message', name=server.get_self_metadata(
    ).name, version=server.get_self_metadata().version, prefix=constants.Prefix)
