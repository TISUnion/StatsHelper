        ...
        for arg in self.__arg_set:
            if len(arg) > 0 and not arg.startswith('-'):
                raise CommandSyntaxError(
                    'Non-arg value "{}" found'.format(content), total_length)
...
class ScoreboardQuery(ArgumentNode):
    def parse(self, text: str) -> ParseResult:
        ...
        remaining = command_builder_util.remove_divider_prefix(
            text[len(arg1):])
        arg2 = command_builder_util.get_element(remaining)
        if len(arg2) > 0 and arg2[0].isalpha():
            # <cls> <target>
            return ParseResult((arg1, arg2), len(text) - len(remaining) + len(arg2))
        else:
            raise UnknownQuickScoreboard(
                'Unknown scoreboard {}'.format(arg1), len(arg1), arg1)
